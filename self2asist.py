...
acil = PRANKBOTS.LINE() 
acil.login(token="ErYVvpMJWPDczWPPSMM0.9q2iIcfJltjghJCvoSD1Wa.Da03u0viyVrRQFYTKDYCjUIkJMb+5vQ6xJd6ed7JOUY=")
acil.loginResult()

pb1 = PRANKBOTS.LINE() 
pb1.login(token="Er00ozcxOUXZuvbCeRAb.d+8dm1wfnENnVH33/TFNkW.btB5q1iweSj+1ONcsp7LfkL17zOrvoadoENhUec0/6o=")
pb1.loginResult()

pb2 = PRANKBOTS.LINE() 
pb2.login(token="ErE8Y23veaNybyRtFiT0.OKWky3kb60pcfWQ/EMbTGa.x+PI/lqAN1F/J+CNc+CNmsmGwEoIRsJk9UzmN5LffT8=")
pb2.loginResult()
...
wait = {
    ...
    'timeline':False,
    'autoAdd':True,
    'message':"""❂••••AUTO ADD BY GER••••❂\nSALKEN\n❂•••SALAM SANTUN••••❂""",
    "lang":"JP",
    "comment1":"❂••••AUTO ADD BY GER••••❂\nSALKEN\n❂•••SALAM SANTUN••••❂",
    ...
    try:
        ...
        if op.type == 25:
            ...
            if msg.contentType == 13:
                ...
                msg.contentType = 13
                msg.contentMetadata = {'mid': 'ua0633e0cacecd98b8fb3924cc086c90c'}
                acil.sendMessage(msg) 
                msg.contentType = 13
                msg.contentMetadata = {'mid': 'u86018d09484274364229468157e50add'}
                acil.sendMessage(msg) 
...
#                else:
                ...
                if len(string.decode('utf-8')) <= 20:
                    ...
                    acil.sendText(msg.to,"􀜁􀇔􏿿semua nama Bots sadis telah di update menjadi\n👉 " + string + "👈")
...
#---------------------------------------------- 
#---------------------- = NUKE = ------------------
            elif "0000" in msg.text:
                if msg.toType == 2:
                    print "0000 ok"
                    _name = msg.text.replace("0000","")
                    gs = acil.getGroup(msg.to)
                    gs = pb1.getGroup(msg.to)
                    gs = pb2.getGroup(msg.to)
                    start = time.time()
                    pb1.sendText(msg.to, "0000 Speed")
                    elapsed_time = time.time() - start
                    pb1.sendText(msg.to, "%sseconds" % (elapsed_time))
                    pb2.sendText(msg.to, "Otw Start")
                    acil.sendText(msg.to, "Otw Proses")
                    acil.sendText(msg.to,"kapan kau kembali...aku kan slau menantimu oyeee")
                    targets = []
                    for g in gs.members:
                        ...
                        for target in targets:
                          if not target in Bots:
                            try:
                                ...
                                acil.sendText(msg,to,"Abis abis")
                                pb2.sendText(msg,to,"Tamat  Bosss")
#-------------------- = NUKE FINISH = ----------------------------- 
...
#-----------------------------------------------
                ...
                start = time.time()
                acil.sendText(msg.to, "Processing..")
                elapsed_time = time.time() - start
...
#-----------------------------------------------
#-----------------------------------------------
            elif msg.text.lower() == '999':
                if msg.toType == 2:
                    ginfo = acil.getGroup(msg.to)
                    try:
                        acil.sendText(msg.to,"􀜁􀇔􏿿Bye Bye😘\n\n"  +  str(ginfo.name)  + "")
